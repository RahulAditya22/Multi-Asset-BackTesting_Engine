...
def sample_csv(tmp_path: Path) -> Path:
    ...
    pd.DataFrame({"Date": pd.date_range("2025-01-01", periods=3), "Open": [10, 11, 12], "High": [11, 12, 13], "Low": [9, 10, 11], "Close": [10.5, 11.5, 12.5], "Volume": [100, 110, 120]}).to_csv(path, index=False)
    return path
...
def test_api_failure_falls_back_to_csv(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("backtester.data_loader.yf.download", fail)
